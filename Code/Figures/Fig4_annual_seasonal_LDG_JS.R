...
dir.create("../../Data/Data_generated") #should already contain df_latGradients_alphaDiv_annual.csv and mOTUsv4.0.gtdb.taxonomy.80mv.tsv.zip and df_latGradients_alphaDiv_month.csv and Wilcox_pairwise_test_june_December.csv
...
# remove prokaryotes
diversity = diversity %>% dplyr::select(-c("prokaryotes"))
## GTDB
...
# Diversity per month
diversity_month = fread("../../Data/Data_generated/df_latGradients_alphaDiv_month.csv")
diversity_month = diversity_month %>% filter(!clade %in% c("prokaryotes")) # drop prokaryotes
# seasonal significance
wilcoxon = fread("../../Data/Data_generated/Wilcox_pairwise_test_june_December.csv")
wilcoxon = wilcoxon %>% filter(!clade %in% c("prokaryotes")) # drop prokaryotes
...
# Step 1: Create taxonomy lookup tables
...
# Step 2: Identify taxonomic level and extract taxon name
div_temp <- div_temp %>%
  mutate(
    level = case_when(
...
# wide format of average diversity
diversity_wide <- div_long_mean %>%
  pivot_wider(names_from = clade, values_from = Annual)
# Compute Pearson correlations (excluding 'y' column)
cor_results <- rcorr(as.matrix(diversity_wide %>% select(-y)), type = "pearson")
...
alpha <- 1 # change optionally
masked_correlation_matrix <- ifelse(p_value_matrix <= alpha, correlation_matrix, NA)

################## Pairwise pearson correlation of clade-level annual LDGS. If phylum is highly similar to corresponding class(es), omit phylum (to remove redundancy) ############################
extract_base_name <- function(name) {
  sub("_class__.*", "", name)
}
# Extract base names for all rows and columns
row_base_names <- sapply(rownames(masked_correlation_matrix), extract_base_name)
col_base_names <- sapply(colnames(masked_correlation_matrix), extract_base_name)
# Identify base names with both short and long versions
unique_base_names <- intersect(row_base_names, col_base_names)
has_long_version <- function(base_name, full_names) {
  any(grepl(paste0("^", base_name, "_class__"), full_names))
}
# Filter base names to include only those with both short and long versions
valid_base_names <- unique_base_names[sapply(unique_base_names, has_long_version, full_names = c(rownames(masked_correlation_matrix), colnames(masked_correlation_matrix)))]
# Initialize a list to store indices to remove
remove_indices <- c()
# Iterate through rows and columns
for (i in seq_along(row_base_names)) {
  for (j in seq_along(col_base_names)) {
    # Check if base names match, correlation > 90%, and base name is valid
    if (row_base_names[i] == col_base_names[j] &&
        row_base_names[i] %in% valid_base_names &&
        !is.na(masked_correlation_matrix[i, j]) &&
        masked_correlation_matrix[i, j] > 0.9) {
      
      # Check if both names belong to the same taxonomic level
      taxonomic_level_i <- ifelse(grepl("_class__", rownames(masked_correlation_matrix)[i]), "class", "phylum")
      taxonomic_level_j <- ifelse(grepl("_class__", colnames(masked_correlation_matrix)[j]), "class", "phylum")
      
      if (taxonomic_level_i == "phylum" && taxonomic_level_j == "class") {
        # Remove the phylum-level name
        remove_indices <- c(remove_indices, i)
      } else if (taxonomic_level_j == "phylum" && taxonomic_level_i == "class") {
        # Remove the phylum-level name
...
# Remove corresponding rows and columns from the matrix
masked_correlation_matrix <- masked_correlation_matrix[-remove_indices, -remove_indices]

## Subset distance matrix
# Get the current labels from the `distance_matrix`
distance_labels <- attr(distance_matrix, "Labels")
# Find the labels corresponding to the `remove_indices` in the correlation matrix
labels_to_remove <- distance_labels[remove_indices]
#labels_to_remove <- c()
# Subset the `distance_matrix` by keeping only the labels not in `labels_to_remove`
labels_to_keep <- setdiff(distance_labels, labels_to_remove)
# Create a new `dist` object with only the kept labels
distance_matrix <- as.dist(as.matrix(distance_matrix)[labels_to_keep, labels_to_keep])
...
# prepare labels
dendro_data$labels$label = gsub(pattern = paste0("_",
                                                 alpha_diversity_metric,"_",
                                                 rarefying_sample_size), replacement = "",
                                dendro_data$labels$label)
dendro_data$labels$label <- gsub("domain__|phylum__|class__", "", dendro_data$labels$label)
dendro_data$labels$label <- gsub(pattern = "_", replacement = " ", dendro_data$labels$label)
## Plot
...
# Extract the first and second words and construct labels with HTML for coloring
dendro_data$labels <- dendro_data$labels %>%
  mutate(
    first_word = sapply(strsplit(label, " "), `[`, 1), # Extract the first word
    second_word = sapply(strsplit(label, " "), `[`, 2), # Extract the second word
    remaining_text = sapply(strsplit(label, " "), function(x) paste(x[-c(1, 2)], collapse = " ")), # Remaining text
    # Use Dark2 palette for first word if it occurs more than once
    first_word_color = ifelse(first_word_freq[first_word] > 1,
                              brewer.pal(n = min(8, length(unique(first_word))), "Dark2")[as.numeric(as.factor(first_word)) %% 8 + 1],
                              "black"),
    # Use Paired palette for second word if it occurs more than once
    second_word_color = ifelse(second_word_freq[second_word] > 1,
                               brewer.pal(n = min(12, length(unique(second_word))), "Paired")[as.numeric(as.factor(second_word)) %% 12 + 1],
                               "black"),
    # Combine colored first and second words with remaining text
    html_label = paste0(
      "<span style='color:", first_word_color, "'>", first_word, "</span> ",
      ifelse(!is.na(second_word), paste0("<span style='color:", second_word_color, "'>", second_word, "</span> "), ""),
      remaining_text
    )
  )
## Dendrogram plot
# control dendrogram height
# Scaling factor for height (e.g. 0.5 = compress by 50%)
height_scale <- 0.2
# Scale segment coordinates
segments_scaled <- segment(dendro_data)
segments_scaled$x <- segments_scaled$x * height_scale
segments_scaled$xend <- segments_scaled$xend * height_scale
# Scale label positions
labels_scaled <- dendro_data$labels
labels_scaled$x <- labels_scaled$x * height_scale

# plot
ggplot(segment(dendro_data)) +
  geom_segment(aes(x = y, y = x, xend = yend, yend = xend)) +
  geom_richtext(data = dendro_data$labels, 
                aes(x = 0, y = x, label = paste0("<b>", html_label, "</b>")), 
                hjust = 0, vjust = 0.5, angle = 0, size = 5, fill = NA, label.color = NA) +
  labs(title = "", x = "", y = "") +
  theme_minimal() +
  theme(
...
# save cladenames that are included in dendrogram
...
wilcoxon <- wilcoxon %>%
  ...
  left_join(phylum_lookup, by = c("taxon_name" = "phylum"), suffix = c("", "_phylum")) %>%
  left_join(domain_lookup, by = c("taxon_name" = "domain"), suffix = c("", "_domain")) %>%
  # Step 3: Reconstruct clade names
  mutate(
    clade = case_when(
      level == "class"  ~ paste0("domain__", domain, "_phylum__", phylum, "_class__", taxon_name),
      level == "phylum" ~ paste0("domain__", domain_phylum, "_phylum__", taxon_name),
